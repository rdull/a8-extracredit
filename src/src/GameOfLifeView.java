...
public class GameOfLifeView extends JPanel implements ActionListener, SpotListener, ChangeListener {
	...
	public GameOfLifeView(int width, int height) {
		...
		this.random_button = new JButton("Randomize");
		...
		submit_panel.setLayout(new BorderLayout());
		submit_panel.add(submit_button, BorderLayout.EAST);
		info_panel_six.add(submit_panel, BorderLayout.SOUTH);
...
//		info_panel_six.add(submit_panel, BorderLayout.SOUTH);
	...
	public void addController(GameOfLifeController controller) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if (button.equals(submit_button)); {
			x = new SubmitEvent("Submit");
		}
		controller.action(x);
	}
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (slider.equals(height_slider)) {
			x = new HeightSlideEvent(slider.getValue());
		} else if (slider.equals(width_slider)) {
			x = new WidthSlideEvent(slider.getValue());
		} else if (slider.equals(low_survive_slider)) {
			x = new LowSurviveSlideEvent(slider.getValue());
		} else if (slider.equals(high_survive_slider)) {
			x = new HighSurviveSlideEvent(slider.getValue());
		} else if (slider.equals(low_birth_slider)) {
			x = new LowBirthSlideEvent(slider.getValue());
		} else if (slider.equals(high_birth_slider)) {
			x = new HighBirthSlideEvent(slider.getValue());
		} else 
		controller.action(x);
	}
